...
**Lock C (Look-ahead bias)**: rolling / expanding-window estimator (MODWT / PCA / similarity scoring / standardization / quantile cut / detrending 등) 는 각 OOS time step 마다 recursive recomputation 의무. 전체 sample fit 후 test 적용 절대 금지. `docs/phase-2_5-verification.md` §3.5 가 자동 검출.
...
- **agent 대안 후보 (2-3개)**:
  1. <대안 A: 어떤 KR 변수 / method / drop 등> — 장단점 + 학술적 사유
  2. <대안 B> — 장단점 + 학술적 사유
  3. <대안 C: drop 후 limitation 으로 명시> — 장단점 + 학술적 사유
- **agent 강한 권장** (1개 선택): <어느 대안 + 1-2 문장 사유 + paper 와의 trade-off>
...
## 5.5 Override mode (학생 명시 자기 책임)

상황: 학생이 지도교수 사전 컨펌 없이 "본인 책임 진행" 선언 (예: session log Turn 9 "교수 승인 override 해서 일단 진행시켜"). agent 는 이를 거부하지 않고 진행하되 다음 의무:

1. `thesis_state/00_feasibility.md` §6 또는 `01_methodology_map.md` 안 **OVERRIDE block** 명시 기록:
   ```
   ### OVERRIDE — <YYYY-MM-DD>
   학생 본인 책임 진행 명시. 지도교수 사전 컨펌 없이 다음 결정 확정:
   - <결정 1>
   - <결정 2>
   - ...
   학생 책임:
   - Phase 3 / 5 self-test 에서 각 결정의 학술적 사유 본인 입으로 설명 의무
   - Phase 4 본문 transparent disclosure 의무
   - 추후 지도교수 reject 시 Phase 1-4 재실행 cost 본인 부담
   ```
2. Phase 3 / 5 self-test 의 §3 Q&A 에 OVERRIDE block 안 각 결정 항목별 forensic 질문 자동 추가.

이는 sign-off 의무 우회가 아니라 "학생 자기 책임 명시" 의 안전망. Lock A/B/C 자체는 약화 X.
